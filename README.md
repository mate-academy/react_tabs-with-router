...
Implement the `App` with `Home` page available at `/` and `Tabs` page
available at `/tabs`. Each page should have the correct title
`Home page` or `Tabs page`. The `Tabs` page should also show a `Tabs` component implemented in [React Tabs](https://github.com/mate-academy/react_tabs#react-tabs).

1. Add a `nav` with `Home` and `Tabs` links
    - It should be visible on every page;
    - An active link should be highlighted with `is-active` class;
